...
public sealed class CushionService
{
    ...
    public Card? FindCushionByName(){
        var c=_cardRepository.GetAll(); 
        return c.FirstOrDefault(z=>z.Name=="Financial cushion");
    }

    public Card? FindCushionByContains(){
        var a=_cardRepository.GetAll();
        return a.FirstOrDefault(q=>q.Name.Contains("cushion",StringComparison.OrdinalIgnoreCase));
    }

    public Card CreateCushion(Currency currency){
        var e=FindCushionByName();
        if(e!=null){
            return e;
        }
        return _cardRepository.Add(
            new Card{Name="Financial cushion",
                Currency=currency,
                InitialBalance=0m,
                IsDefault=false,
                IsCushion=true
    ...
    public decimal DefaultTransferAmount(decimal incomeAmount,string category){
        var s=category.Contains("Salary",StringComparison.OrdinalIgnoreCase);
        if(incomeAmount<10m){
            if(s){
                return 1m;
            }return 1m;
